/* Vendored, unmodified, from PARENA's own runtime/parena_runtime.h (public domain, Unlicense --
 * PARENA/LICENSE) -- the real C runtime the generated cannon module (see the cannon directory's
 * own _gen.c files, docs/LO_CANNON_PROGRAMMING.md, EMILY/BACKLOG.md SECTION 549) links against,
 * same "port sibling-repo infra, trim to D2's real needs" precedent core/net.h/http.{h,c}/
 * iduna.{h,c} and core/palette.h/font.{h,c}/sfx.{h,c} already established. D2 always builds with
 * PARENA_NO_GRAPHICS defined (see scripts/build.sh) -- this header's own documented escape hatch
 * (see its own "SDL2 -- built in" comment) skips the SDL2/SDL2_ttf include entirely, since D2's
 * cannon decision functions are pure base4 I32 logic with zero graphics surface. Regenerate by
 * re-copying from a PARENA checkout if PARENA's own runtime changes -- do not hand-edit. */
